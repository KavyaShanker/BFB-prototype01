...
w_RT = 1  # Adjust based on importance
w_RIT = 1  # Adjust based on importance
...
essential_attributes = ['Product Name', 'Brand Name', 'Price', 'Quantity/Size', 'Shade/Color', 'Ingredients', 
                        'Product Type', 'Usage Instructions', 'Expiration Date', 'Manufacturing Date', 'Country of Origin', 
...
def calculate_image_quality(image_path):
    """Calculate image quality based on resolution, lighting, and consistency."""
    try:
        img = Image.open(image_path)
        np_img = np.array(img)
        ...
        resolution_score = (img.width * img.height) / (300 * 300)
        resolution_score = min(resolution_score, 1)

        # Lighting Quality
        gray_img = cv2.cvtColor(np_img, cv2.COLOR_RGB2GRAY)
        lighting_score = np.std(gray_img) / 255

        # Consistency Quality Placeholder
        consistency_score = 1
        
        wres, wlight, wcons = 0.4, 0.3, 0.3
        image_quality_score = (wres * resolution_score +
                               wlight * lighting_score +
                               wcons * consistency_score)
...
def calcCompleteness(data):
  ...
  for index, row in data.iterrows():
    LA = calculate_LA(row)
    LD = calculate_LD(row)
    LIQ = calculate_image_quality(row['Image Path'])
...
# # df = pd.read_csv('bfbfiles/average_case.csv')
...
#   # Combine individual scores into a DataFrame
#   individual_scores = pd.DataFrame({
#       'Correctness Score': correctness_scores,
...
#   # Add image quality assessment as a separate column (assuming basic_image_quality_assessment returns a value between 0 and 1)
#   image_quality = [basic_image_quality_assessment(row['Image Path']) for index, row in data.iterrows()]
#   individual_scores['Image Quality'] = image_quality

#   # Combine all scores into a single DataFrame
#   all_scores = pd.concat([individual_scores, total_score.rename('Total Score')], axis=1)
...
def main(data):
    ...
    data['Correctness'] = individual_scores['Correctness Score']
    data['Completeness'] = individual_scores['Completeness Score']
    data['Compliance'] = individual_scores['Compliance Score']
    # data['Image Quality'] = individual_scores['Image Quality']
    data['Total Score'] = all_scores['Total Score']
